...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('phoneContactToggle', () => ({
        ...
        init() {
            // Thu thập các card và ẩn bớt sau khi DOM sẵn sàng
            this.$nextTick(() => {
                const grid = this.$el.querySelector('.phone-contact-grid');
                if (!grid) return;
        ...
        hideAdditionalItems() {
            this.items.forEach((item, index) => {
                item.style.display = index < this.initialItems ? 'block' : 'none';
            });
        },
        ...
        getButtonText() {
            ...
            const hidden = Math.max(total - this.initialItems, 0);
            return this.isExpanded ? 'Thu gọn' : `Xem Thêm (${hidden} số khác)`;
        },
